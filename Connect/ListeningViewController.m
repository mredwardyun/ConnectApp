...
- (void)listenToBroadcasts {
	NSString *messageReceived = self.messageReceived;
	NSLog(@"listenToBroadcasts message received %@", messageReceived);
	...
	for (NSString *component in components) {
		...
		if ([component containsString:@"FB:"]) {
			self.contactUsername = [component componentsSeparatedByString:@"FB:"][1];
		}
	}
	NSLog(@"listentobroadcasts name %@ fb %@ contactavailableservices %@", self.contactRealname, self.contactUsername, self.contactAvailableServices);
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
	...
	if ([stringFromData isEqualToString:@"EOM"]) {
		...
		self.messageReceived = [[NSString alloc] initWithData:self.data encoding:NSUTF8StringEncoding];
